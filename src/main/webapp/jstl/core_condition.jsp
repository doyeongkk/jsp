...
<body>
    ...
    <%-- java
    		...
    		1. test 속성 안에 조건을 기술할때는 EL식에 포함해서 작성을 해야한다. 
    		%{userid } == "brown" 	X
    		${userid } == 'brown' 	X
    		${userid == 'brown' }   O
    ...
    --%>
      ...
      <%
          if( ((String)pageContext.getAttribute("userid")).equals("brown")){
            out.write("userid-스트립틀릿:" +(String)pageContext.getAttribute("userid"));
          }
       %>
    <br>
    ...
    <c:if test="${userid =='brown'}">  <!-- }"사이에 공백있으면 안된다.  -->
     	올바른예 userid : ${userid }<br>
    </c:if>
    
    <h3>choose:일반if, else if, else</h3>
    <c:choose>
       ...
       <c:when test="${userid == 'conny' }">conny 사용자 입니다</c:when>
