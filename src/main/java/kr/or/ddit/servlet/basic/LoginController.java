...
/* web.xml에 설정하는 servlet, servlet-mapping을 통해 설정하는 방법*/
...
protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
	...
	// 클라이언트가 서버로 요청으 보낼 시 브라우저에 의해 같이 전송된 쿠키 정보 확인
    ...
    // userId가 brown일 때 비빌번호가 brownpass 인경우에 한해 로그인이 성공되었다고 판단
    // 그 외 경우는 로그인 실패로 간주 
    // 로그인 성공시: main.jsp로 forward
    // forward: 요청을 처리할 다른 jsp, servlet에게 위임
    //  		서버안에서 이루어지는 작업으로 클라이언트 입장에서는 누가 응답을 생성 했는지 알 수 없다.
    //			request.getRequestDespatcher(url)를 이용하여 requestDispatcher
